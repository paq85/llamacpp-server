#!/usr/bin/env python3
"""Benchmark llama.cpp for agentic-coding (code generation) on the PAQ_LLAMACPP_SERVER/Qwen
Q5 27B host. Uses a >=50K-token prompt for both prefill and decode so the
measurement reflects large agent contexts. Proxy is disabled per variant
(CLOUDFLARE_TIMEOUT_PROXY_MODE=off).

Sweeps:
  A) MTP n_max (draft-mtp) n1..n6 (+ off) with vision on+offloaded (production).
  B) Vision config: on+offloaded, on+CPU, off  (with MTP n2).

GPU memory used/free is captured per variant so configurations that would push
model+KV beyond VRAM can be flagged / excluded.
"""
...
DEFAULT_PROMPT_TOKENS = 51200  # >=50K agent context
...
def variants_default() -> list[Variant]:
    mtp = [
        Variant("mtp_off", {"SPEC_TYPE": "off"}),
        *(
            Variant(f"mtp_n{n}", {"SPEC_TYPE": "draft-mtp", "SPEC_DRAFT_N_MAX": str(n)})
            for n in range(1, 7)
        ),
    ]
    vision = [
        Variant("vision_on_offloaded", {}),                                  # inherits ENABLE_MMPROJ=1, MMPROJ_OFFLOAD=1
        Variant("vision_on_cpu", {"MMPROJ_OFFLOAD": "0"}),                   # vision on, mmproj on CPU
        Variant("vision_off", {"ENABLE_MMPROJ": "0"}),                       # vision disabled
    ]
    return mtp + vision
...
def https_json(base_url: str, path: str, api_key: str, payload: dict[str, Any] | None = None, timeout: float = 1200.0) -> dict[str, Any]:
...
def run_pp(base_url: str, api_key: str, prompt: str, seed: int) -> dict[str, Any]:
    ...
    resp = https_json(base_url, "/completion", api_key, payload, timeout=1200.0)
...
def run_tg(base_url: str, api_key: str, prompt: str, seed: int, n_predict: int) -> dict[str, Any]:
    payload = {
        "prompt": prompt,
        "n_predict": n_predict,
        "temperature": 0.6,
...
def run_variant(variant: Variant, args, api_key: str, timestamp: str) -> dict[str, Any]:
    ...
    log_path = RESULTS_DIR / f"{timestamp}-agent-{variant.name}.log"
    env = os.environ.copy()
    env.update({
        "HOST": "127.0.0.1",
        "PORT": str(port),
    ...
    try:
        ...
        # Warmup (small prompt) primes kernels.
        ...
        # Decode at >=50K context (same prompt, then generate).
        tg_runs = [run_tg(base_url, api_key, prompt, seed=args.seed + 200 + i, n_predict=args.tg_tokens) for i in range(args.tg_runs)]
...
def render_markdown(results: list[dict[str, Any]]) -> str:
    rows = sorted(results, key=lambda r: r.get("summary", {}).get("tg_tps_mean", 0) or 0, reverse=True)
    lines = [
    ...
    for rank, r in enumerate(rows, 1):
        ...
        lines.append(
            f"| {rank} | `{r['variant']}` | {s.get('tg_tps_mean', 0):.2f} | {s.get('pp_tps_mean', 0):.1f} | "
            f"{s.get('tg_acceptance_mean', 0):.2%} | {s.get('pp_tokens', 0):.0f} | {used} |"
        )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    p.add_argument("--prompt-tokens", type=int, default=DEFAULT_PROMPT_TOKENS, help="Prompt length (>=50000 recommended)")
    ...
    p.add_argument("--startup-timeout", type=float, default=360.0)
    p.add_argument("--variants", default="", help="Comma list of variant names; default all")
    args = p.parse_args()

    if systemd_service_is_active(SYSTEMD_UNIT):
        raise SystemExit(
            f"{SYSTEMD_UNIT} is active. Stop the systemd-managed server before running agent benchmarks "
    ...
    print(f"Benchmarking {len(chosen)} variants on port {args.port}: prompt={args.prompt_tokens} tg={args.tg_tokens}x{args.tg_runs}")
    ...
    json_path = RESULTS_DIR / f"{timestamp}-agent.json"
    ...
    md_path = RESULTS_DIR / f"{timestamp}-agent.md"
